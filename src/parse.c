...
node_t *parse_input(char *command_string)
{
	if (*command_string == '&')
		return NULL;
	...
	default:
		...
		list_ptr->command =
		    calloc(parsed_expression.we_wordc + 1, sizeof(char **));

		for (size_t i = 0; i < parsed_expression.we_wordc; ++i) {
			list_ptr->flags =
			    get_type(parsed_expression.we_wordv[i]);
...
int get_type(char *token)
{
	...
	switch (*token) {
	case '|':
		return NODE_REDIRECTION_PIPE;
	case '&':
		return NODE_EXEC_ASYNC;
	case '<':
		return (NODE_REDIRECTION | NODE_REDIRECTION_READ);
...
void escape_char(char *string, char escape)
{
